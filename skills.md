...
- **Theme**: flat warm palette. Default (light): page AND boxes are all greige `#c3c2b7` — the `#8f8d80` border/muted does the separation — text `#1e1e1d`, accent `#d57455` (hover `#a85536`, tint `#f0d9cd`), inputs off-white `#f7f4ef`. Dark preset: everything `#1e1e1d` (inputs `#2a2a28`), text `#f7f4ef`, muted `#c3c2b7`, same terracotta accent. The "🎨 Theme" modal offers Light/Dark presets + custom accent/bg/text (hex; border/muted/deep/input derived by luminance, surface always = bg for the flat look), persisted in localStorage. The native app accepts the same three hex colors in Settings (prefs → `AppTheme`; cards tinted = bg). The QR renders charcoal-on-greige with a scannability guard.
