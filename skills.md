...
- **Theme**: flat warm palette in the `#d57455` family. Default (light): page AND boxes are terracotta-tinted warm white `#f9f1ec` with `#e0c9bd` borders and warm-taupe muted `#8f7364`, text `#1e1e1d`, accent `#d57455` (hover `#a85536`, tint `#f0d9cd`), inputs `#fffdfb`. Dark preset: everything `#1e1e1d` (inputs `#2a2a28`), text `#f7f4ef`, muted `#c3c2b7`, same terracotta accent. The "🎨 Theme" modal offers Light/Dark presets + custom accent/bg/text (hex; border/muted/deep/input derived by luminance, surface always = bg for the flat look), persisted in localStorage. The native app's Settings has the same ☀️ Light / 🌙 Dark preset buttons plus the three custom hex fields (prefs → `AppTheme`; cards tinted = bg; untagged labels/EditTexts remapped by XML color so Dark stays readable). The QR renders charcoal-on-warm-white with a scannability guard.
