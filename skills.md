...
- **Theme**: flat warm palette in the `#d57455` family, identical on web and native. Light (the default): page, boxes, and inputs are all `#f9f1ec` with `#e0c9bd` borders, muted `#8f7364`, text `#1e1e1d`, accent `#d57455` (hover `#a85536`, tint `#f0d9cd`). Dark preset: everything `#1e1e1d` (inputs `#2a2a28`, borders `#474641`), text `#f7f4ef`, muted `#c3c2b7`, same terracotta. "Light" restores the exact defaults (web removes the CSS-var overrides; native clears the theme prefs) rather than re-deriving, so default == light always. The "🎨 Theme" modal / native Settings offer Light/Dark presets + custom accent/bg/text hex (other tones derived by luminance; surface always = bg). Native recoloring uses `GradientDrawable.setColor`+`setStroke` — NOT `setTint`, which paints over the stroke and erases borders. The modal's hex fields sync to whichever preset is active so Apply never silently reverts to light values.
- **Toolbar layout**: the tools card is two rows — search + "🌐 All folders" + sort on top, then a bordered button row (New folder / Trash / Camera / Theme). The QR renders charcoal-on-warm-white with a scannability guard.
